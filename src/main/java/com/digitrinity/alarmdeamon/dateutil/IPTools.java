...
{    
    ...
    private static Logger logger = LogManager.getLogger(IPTools.class.getName());
    ...
    public static class IPAddressList
    {
        ...
        public java.util.List getIPList() {
            if (this.ipList == null) { this.ipList = new Vector(); }
            return this.ipList;
        }
        
        public void addIPAddress(IPAddress ipAddr) {
            java.util.List v = this.getIPList();
        ...
        public boolean contains(String ipAddr) {
            for (Iterator i = this.getIPList().iterator(); i.hasNext();) {
    ...
    {
        logger.debug("Match? " + ip + " => " + ipal.isMatch(ip));
    }
    ...
    public static void main(String argv[])
    {
        ...
        if (argv.length > 0) {
        	logger.debug("Checking " + argv[0]);
            if (ntip.contains(argv[0])) {
            	logger.debug(argv[0] + " IS in the list!");
            } else {
            	logger.debug(argv[0] + " is NOT in the list.");
            }
        }
    }
